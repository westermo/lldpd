...
static int
cmd_port_desc(struct lldpctl_conn_t *conn, struct writer *w,
		struct cmd_env *env, void *arg)
{
	lldpctl_atom_t *port;
	const char *name;
    ...
    struct cmd_node *unconfigure)
{
	...
	commands_new(
		commands_new(
			commands_new(configure_lldp,
					"portdescription", "Port Description", NULL, NULL, "ports"),
				NULL, "The description", NULL, cmd_store_env_value, "port-descr"),
			NEWLINE, "Set Port Description", NULL, cmd_port_desc, NULL);
